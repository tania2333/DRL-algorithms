...
class BiCNet:
    ...
    def actor_build_network(name, observation, num_agents, hidden_vector_len, output_len):
        with tf.variable_scope(name):
            ...
            return rnn_out, outputs#tf.clip_by_value(outputs, -1, 1)#outputs#  tf.clip_by_value(outputs, -1, 1)
    ...
    def shared_dense_layer_tanh(name, observation, num_agents, output_len):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            for j in range(num_agents):
                ...
                # agent_encoded = tf.contrib.layers.fully_connected(agent_obs, output_len, activation_fn=tf.nn.tanh, weights_initializer=tf.contrib.layers.xavier_initializer(),biases_initializer=tf.contrib.layers.xavier_initializer())  # always (batches, 1)  out_len:输出的维度大小，outputs会改变inputs的最后一维为output_len
                agent_encoded = tf.layers.dense(agent_obs, output_len, name="dense", activation=tf.nn.tanh)
    ...
    def shared_dense_layer(name, observation, num_agents, output_len):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            for j in range(num_agents):
                agent_obs = observation[:, j]  # （batches, 1) or 2
                # agent_encoded = tf.contrib.layers.fully_connected(agent_obs, output_len, activation_fn=None, weights_initializer=tf.contrib.layers.xavier_initializer(),biases_initializer=tf.contrib.layers.xavier_initializer())  # always (batches, 1)  out_len:输出的维度大小，outputs会改变inputs的最后一维为output_len  , kernel_initializer=tf.random_normal_initializer(mean=0, stddev=1),bias_initializer=tf.random_normal_initializer(mean=0, stddev=1)
