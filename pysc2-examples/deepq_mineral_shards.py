...
import baselines.deepq.utils as U_b
...
_PLAYER_RELATIVE = features.SCREEN_FEATURES.player_relative.index  ## 5
_PLAYER_FRIENDLY = 1       # team member
...
def learn(env,
  ...
  def make_obs_ph(name):  # Creates a placeholder for a batch of tensors of a given shape and dtype
  ...
  obs = env.step(actions=[sc2_actions.FunctionCall(_SELECT_ARMY, [_SELECT_ALL])])  # Apply actions, step the world forward, and return observations.

  # 查看返回的字典中屏幕中的目标关系分布图：1表示着地图中个体的位置，3表示着矿物的位置,就是终端的矩阵图
  player_relative = obs[0].observation["feature_screen"][_PLAYER_RELATIVE]  #obs is a 'TimeStep' whose type is tuple of ['step_type', 'reward', 'discount', 'observation'];step_type.first or mid or last
  # 矿的位置 0,1矩阵分布
  screen = (player_relative == _PLAYER_NEUTRAL).astype(int) #+ path_memory   screen=1 or 0  to indicate the location of mineral
  # 队友的位置,给出行列信息
  player_y, player_x = (player_relative == _PLAYER_FRIENDLY).nonzero()  #the location of team member: row, col <-> y,x
    ...
    for t in range(max_timesteps):
      ...
      if t > learning_starts and t % train_freq == 0:   #train_freq=1: update the model every `train_freq` steps
        ...
        if prioritized_replay:
          ...
          weights_x, batch_idxes_x = np.ones_like(rewards_x), None  # weights_x is an array padded with 1 which has the same shape as rewards_x
        ...
        td_errors_y = train_y(obses_t_y, actions_y, rewards_y, obses_tp1_y, dones_y, weights_y)
      ...
      mean_100ep_reward = round(np.mean(episode_rewards[-101:-1]), 1)  # round: sishewuru value
