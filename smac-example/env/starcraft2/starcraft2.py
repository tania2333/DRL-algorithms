...
class StarCraft2Env(MultiAgentEnv):
    ...
    def get_obs_agent(self, agent_id):
        ...
        move_feats = np.zeros(move_feats_len, dtype=np.float32) #move_feats_len=4    ->4
        enemy_feats = np.zeros((self.n_enemies, nf_en), dtype=np.float32)  #n_enemies=8,nf_en=5          ->40  44
        ally_feats = np.zeros((self.n_agents - 1, nf_al), dtype=np.float32) #nf_al=5        ->35           79
        own_feats = np.zeros(nf_own, dtype=np.float32) #nf_own=1       ->1   80
    ...
    def get_state(self):
        ...
        for e_id, e_unit in self.enemies.items():
            ...
            if e_unit.health > 0:
                ...
                if(enemy_state[e_id, 0] < min):
                    min = enemy_state[e_id, 0]
                    target_attack = e_id
        ...
        state = np.append(ally_state.flatten(), enemy_state.flatten())  #[all0, all1, all2 ,all3; x,x,x,x;...//en0,en1,en2; en0',en1',en2'] len=56
        if self.state_last_action:   # True!!!!!!!!!!
