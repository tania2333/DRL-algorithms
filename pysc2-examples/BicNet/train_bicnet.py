...
class ActorNetwork(object):
    def __init__(self, sess, learning_rate, tau, batch_size, num_agents, vector_obs_len, output_len, hidden_vector_len):
        ...
        self.inputs, self.out, self.rnn_out = self.create_actor_network("actor_network")  #in: (,2,1) out(,2,1)/ (?,3,1)
        self.network_params = tf.trainable_variables()   # print('param',len(self.network_params))   8

        self.target_inputs, self.target_out, self.target_rnn_out = self.create_actor_network("target_actor_network")
    ...
    def create_actor_network(self, name):
        ...
        # print('input',inputs)
        rnn_out, out = CommNet.actor_build_network(name, inputs, self.num_agents, self.hidden_vector_len, self.output_len)   #(, 2,1)
        return inputs, out, rnn_out
    ...
    def rnn_out_pre(self, inputs):
        return self.sess.run(self.rnn_out, feed_dict={
            self.inputs: inputs
        })
...
class CriticNetwork(object):
    ...
    def __init__(self, sess, learning_rate, tau, gamma, num_actor_vars, num_agents, vector_obs_len, output_len, hidden_vector_len):
        self.sess = sess
        self.learning_rate = learning_rate
        self.tau = tau
        self.gamma = gamma
        ...
        self.inputs, self.action, self.out,self.rnn_c_out = self.create_critic_network("critic_network")
        self.network_params = tf.trainable_variables()[num_actor_vars:]

        self.target_inputs, self.target_action, self.target_out,self.rnn_c_target_out = self.create_critic_network("target_critic_network")
    ...
    def create_critic_network(self, name):
        ...
        out,rnn_c_out = CommNet.critic_build_network(name, inputs, action, self.num_agents, self.hidden_vector_len, self.output_len)
        return inputs, action, out, rnn_c_out
    ...
    def predict_target_rnn(self, inputs, action):
        return self.sess.run(self.rnn_c_target_out, feed_dict={
            self.target_inputs: inputs,
            self.target_action: action
        })
