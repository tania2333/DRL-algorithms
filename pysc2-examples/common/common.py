...
def init(env, obs):
  ...
  # group_id为9，因此最大编组10个单位后退出
  for i in range(len(player_x)):

    if group_id > 9:
      break

    xy = [player_x[i], player_y[i]]   #我方单位位置
    unit_xy_list.append(xy)      #存放我方单位位置的列表

    if (len(unit_xy_list) >= 1):
      for idx, xy in enumerate(unit_xy_list):
        if (idx == 0):
          obs = env.step(actions=[
            sc2_actions.FunctionCall(_SELECT_POINT, [[0], xy])  #0: select
          ])
        else:
          obs = env.step(actions=[
            sc2_actions.FunctionCall(_SELECT_POINT, [[1], xy])  #1: toggle
          ])
        last_xy = xy

      obs = env.step(actions=[
        sc2_actions.FunctionCall(_SELECT_CONTROL_GROUP,
                                 [[_CONTROL_GROUP_SET], [group_id]]) #_CONTROL_GROUP_SET=1:set
      ])
      unit_xy_list = []
      xy_per_marine[str(group_id)] = last_xy   #最后一个单位的位置信息存放进去
...
def select_marine(env, obs):
  ...
  if (danger_min_dist != None and danger_min_dist <= 5): #寻找我方单位与所有敌方单位的最短距离，如果最短距离小于安全距离，该单位被选中；
    obs = env.step(actions=[
  ...
  elif (marine_closest != None and marine_min_dist <= 3): #计算我方单位之间的距离，如果距离过近小于一定值，那么选中其中一个单位
    obs = env.step(actions=[
...
def marine_action(env, obs, player, action):
  ...
  if (closest == None):
    ...
    #print('coord',coord)         这边应该按照32处理而不是64！！！！！！！！！！！！！！！！！！！！！
    if (coord[0] < 0):
      coord[0] = 0
    elif (coord[0] > 63):
      coord[0] = 63
